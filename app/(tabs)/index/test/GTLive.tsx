...
type SubjectSection = { subjectId: string; subjectName: string; coverImage: any; questions: Question[]; sectionType: 'Easy' | 'Hard'; };
type SubjectBreakdown = { easy: Question[], hard: Question[], name: string, id: string, coverImage: any };

// ===================================================================
// SUB-COMPONENT: BriefingScreen
// ===================================================================
const BriefingScreen = ({
  session, onStart, totalSections, timeLeft
}: {
  session: SubjectSection; onStart: () => void; totalSections: number; timeLeft: number;
}) => {
    // Assuming equal time distribution for display purposes
    ...
    <View style={styles.stateContainer}>
        ...
        <MotiView style={styles.briefingFooter} from={{ translateY: 100 }} animate={{ translateY: 0 }} transition={{ type: 'spring', damping: 20 }}>
            <TouchableOpacity onPress={onStart} style={styles.startButton} activeOpacity={0.8}><Text style={styles.startButtonText}>Begin Section</Text><Ionicons name="arrow-forward" size={24} color="#0A1710"/></TouchableOpacity>
        </MotiView>
    </View>
)};

// ===================================================================
// SUB-COMPONENT: ReviewScreen
// ===================================================================
const ReviewScreen = ({ onContinue, subjectBreakdown, answers, baseGlobalIndex, isFinalReview }: {
    ...
    const [breakSeconds, setBreakSeconds] = useState(600); // 10 minutes
            ...
            <MotiView from={{opacity:0, scale:0.9}} animate={{opacity:1, scale:1}} style={styles.reviewHeaderCard}>
                ...
                <View style={styles.breakTimerContainer}>
                    ...
                    <Text style={styles.breakTimerText}>
                         {breakSeconds > 0 ? `Break Time: ${mins}:${secs.toString().padStart(2, '0')}` : "Break Over!"}
                    </Text>
                </View>
            </MotiView>
            {allQuestions.map((q:Question, idx:number) => {
                const userAnswer = answers[baseGlobalIndex + idx]; const isCorrect = userAnswer === q.answer;
                return (
                    <MotiView key={`review-${q.question.slice(0, 10)}-${idx}`} from={{opacity: 0, translateX: -20}} animate={{opacity:1, translateX:0}} transition={{type: 'timing', delay: 100 + idx*50}} style={[styles.reviewQuestionCard, {borderColor: isCorrect ? '#2a5a3a' : '#8c2b2b'}]}>
                        ...
                        {!isCorrect && <View style={[styles.answerRow, {marginTop: 8, backgroundColor:'rgba(56, 224, 123, 0.1)'}]}>
             ...
             <TouchableOpacity onPress={onContinue} style={[styles.startButton, {marginBottom: 40, marginTop: 16}]}><Text style={styles.startButtonText}>{isFinalReview ? 'Finish & See Results' : 'Continue'}</Text></TouchableOpacity>
        </ScrollView>
    )
};

// ===================================================================
// SUB-COMPONENT: TestingScreen
// ===================================================================
const TestingScreen = ({ question, onAnswer, globalQuestionNumber, totalQuestions, onNext, onPrev, isLast, isFirst, currentAnswer }: {
    question: Question; onAnswer: (option: string) => void; globalQuestionNumber: number; totalQuestions: number; onNext: () => void; onPrev: () => void; isLast: boolean; isFirst: boolean; currentAnswer: string | null;
}) => {
    return (
        <View style={[styles.stateContainer, {paddingHorizontal: 0}]}>
            ...
            <View style={styles.navButtons}>
                <TouchableOpacity onPress={onPrev} disabled={isFirst} style={[styles.navButton, isFirst && {opacity: 0.5}]}><Ionicons name="chevron-back" size={24} color="#E0F0E8"/></TouchableOpacity>
                <TouchableOpacity onPress={onNext} style={[styles.navButton, {backgroundColor: '#38e07b'}]}><Text style={[styles.navText, {color: '#0A1710'}]}>{isLast ? 'Finish & Review' : 'Next'}</Text><Ionicons name="chevron-forward" size={24} color="#0A1710"/></TouchableOpacity>
            </View>
        </View>
    );
};
...
export default function GTLiveScreen() {
    ...
    const { sessions, allQuestions, totalQuestions, allSubjectBreakdowns } = useMemo(() => {
        try {
            const structuredExam = JSON.parse(params.exam as string);
            const s: SubjectSection[] = [];
            let q: Question[] = [];
            const breakdowns: { [key: string]: SubjectBreakdown } = {};
            ...
            structuredExam.forEach((subject: any) => {
                ...
                if (subject.easy?.length > 0) { s.push({ ...subject, questions: subject.easy, sectionType: 'Easy' }); q = q.concat(subject.easy); breakdowns[subject.subjectId].easy = subject.easy; }
                if (subject.hard?.length > 0) { s.push({ ...subject, questions: subject.hard, sectionType: 'Hard' }); q = q.concat(subject.hard); breakdowns[subject.subjectId].hard = subject.hard; }
            });
    ...
    const handleSubmit = async () => {
        ...
        const completedTest = {
            id: `gt_${testIndex}_${new Date().toISOString()}`, name: `${testName} GT`, dateCompleted: new Date().toISOString(), score: finalScore, maxScore: Number(params.maxScore), subjects: completedSubjects,
        };
        ...
        try {
            ...
            Alert.alert("Save Error", "Could not save your test result.");
        }
    ...
    const handleAnswer = (option: string) => {
        setAnswers(prev => { const newAnswers = [...prev]; newAnswers[globalQuestionIndex] = option; return newAnswers; });
    };
    ...
    const handleNext = () => {
        ...
        if (!isLastQuestionInSession) {
            ...
            const isHardSection = currentSession.sectionType === 'Hard';
            // Review if it's a hard section, OR if the next session belongs to a new subject (or there is no next session)
            const nextSessionStartsNewSubject = currentSessionIndex + 1 >= sessions.length || sessions[currentSessionIndex].subjectId !== sessions[currentSessionIndex + 1]?.subjectId;
            
            if (isHardSection || nextSessionStartsNewSubject) {
                setExamState('review');
            } else {
                // Otherwise, move to the briefing for the next section (i.e., Easy -> Hard)
    ...
    const handlePrev = () => {
        if (currentQuestionIndex > 0) {
            ...
            setCurrentQuestionIndex(sessions[prevSessionIndex].questions.length - 1);
            setExamState('testing'); // Go directly to testing, not briefing, when going back
        }
    };

    const handleContinueFromReview = () => {
        // Find the index of the first session belonging to the next subject
        const nextSubjectSessionIndex = sessions.findIndex(s => s.subjectId !== currentSession.subjectId && sessions.indexOf(s) > currentSessionIndex);
        if (nextSubjectSessionIndex !== -1) {
            setCurrentSessionIndex(nextSubjectSessionIndex);
            setCurrentQuestionIndex(0);
            setExamState('briefing');
        } else {
            // This was the last subject review
            handleSubmit();
        }
    };
    
    if (totalQuestions === 0) {
        // Handle case where exam data failed to parse, avoids crashing.
        return (
            <SafeAreaView style={styles.safeArea}>
                <View style={{flex: 1, alignItems:'center', justifyContent: 'center'}}>
                    <Text style={styles.reviewTitle}>Error Loading Test</Text>
                </View>
            </SafeAreaView>
        );
    }
    ...
    // Derived states for props
    const isLastQuestionOfAll = currentSessionIndex === sessions.length - 1 && currentQuestionIndex === currentSession.questions.length - 1;
    const isFirstQuestionOfAll = currentSessionIndex === 0 && currentQuestionIndex === 0;
    const isFinalReview = sessions.findIndex(s => s.subjectId !== currentSession.subjectId && sessions.indexOf(s) > currentSessionIndex) === -1;
    // Calculate base index for the subject review screen
    const firstSessionOfThisSubjectIndex = sessions.findIndex(s=>s.subjectId === currentSession.subjectId);
    const baseGlobalIndexForReview = getGlobalIndex(firstSessionOfThisSubjectIndex, 0);


    return (
        <View style={{flex:1}}>
            <Image source={require('@/assets/images/dark-pattern.png')} style={styles.bgPattern} resizeMode='repeat'/>
            <LinearGradient colors={['rgba(26, 51, 37, 0.95)', '#0A1710']} style={StyleSheet.absoluteFill} />
            <SafeAreaView style={styles.safeArea}>
                <MotiView style={styles.header} from={{translateY: -100}} animate={{translateY: 0}} transition={{type: 'spring', damping: 20}}>
                    <TouchableOpacity onPress={() => router.back()} style={{padding:5}}><Ionicons name="close-outline" size={28} color="#87B59A"/></TouchableOpacity>
                    {currentSession && <View style={styles.headerSubject}><Image source={currentSession.coverImage} style={styles.subjectIcon}/><Text style={styles.headerSubjectText} numberOfLines={1}>{currentSession.subjectName}</Text></View>}
                    <View style={styles.timer}><Ionicons name="time-outline" size={20} color="#87B59A"/><Text style={styles.timerText}>{minutes}:{seconds.toString().padStart(2, '0')}</Text></View>
                </MotiView>
                
                <AnimatePresence exitBeforeEnter>
                    {examState === 'briefing' && currentSession && <BriefingScreen key="briefing" session={currentSession} onStart={() => setExamState('testing')} totalSections={sessions.length} timeLeft={secondsLeft} />}
                    {examState === 'testing' && currentQuestion && <TestingScreen key={`testing-${globalQuestionIndex}`} question={currentQuestion} onAnswer={handleAnswer} globalQuestionNumber={globalQuestionIndex + 1} totalQuestions={totalQuestions} onNext={handleNext} onPrev={handlePrev} isLast={isLastQuestionOfAll} isFirst={isFirstQuestionOfAll} currentAnswer={answers[globalQuestionIndex]}/>}
                    {examState === 'review' && currentSession && <ReviewScreen key="review" onContinue={handleContinueFromReview} subjectBreakdown={allSubjectBreakdowns[currentSession.subjectId]} answers={answers} baseGlobalIndex={baseGlobalIndexForReview} isFinalReview={isFinalReview}/>}
                </AnimatePresence>
            </SafeAreaView>
        </View>
    );
