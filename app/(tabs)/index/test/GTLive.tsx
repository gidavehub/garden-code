...
export default function GTLiveScreen() {
    ...
    const { sessions, allQuestions, totalQuestions, allSubjectBreakdowns } = useMemo(() => {
        try {
            if (!params.exam || typeof params.exam !== 'string') {
                throw new Error("Exam parameter is missing or not a string.");
            }
            ...
            structuredExam.forEach((subject: any) => {
                if (!breakdowns[subject.subjectId]) {
                    breakdowns[subject.subjectId] = { easy: [], hard: [], name: subject.subjectName, id: subject.subjectId, coverImage: subject.coverImage };
                }
                if (subject.easy?.length > 0) {
                    s.push({ ...subject, questions: subject.easy, sectionType: 'Easy' });
                    q = q.concat(subject.easy);
                    breakdowns[subject.subjectId].easy = subject.easy;
                }
                if (subject.hard?.length > 0) {
                    s.push({ ...subject, questions: subject.hard, sectionType: 'Hard' });
                    q = q.concat(subject.hard);
                    breakdowns[subject.subjectId].hard = subject.hard;
                }
            });
    ...
    const handleSubmit = async () => {
        ...
        const completedSubjects = Object.values(allSubjectBreakdowns).map(subject => {
            ...
            return {
                ...
                questions: [...subject.easy, ...subject.hard],
        ...
        try {
            ...
            if (isOnline) {
                // *** THE FIX IS HERE ***
                // Retrieve user data from AsyncStorage to get the email reliably.
                ...
                if (userJson) {
                    ...
                    if (userEmail) {
                        const completedTestForFirestore = {
                            ...completedTest,
                            subjects: completedTest.subjects.map(s => ({ ...s, coverImage: null })), // Don't sync images
                        };
                        ...
                        console.log('Email not found in stored user data. Cannot save to Firestore.');
                    }
                } else {
                    console.log('User not logged in or data not found in storage. Cannot save to Firestore.');
                }
            } else {
                console.log('Offline. Test saved locally, will sync later.');
            }
        } catch (e) {
    ...
    const handleCheatSubmit = async () => {
        ...
        const finalScore = 800; // Hardcoded cheat score
        ...
        const completedTest = {
            id: `gt_${testIndex}_${new Date().toISOString()}`,
            name: `${testName} GT (Cheated)`,
            dateCompleted: new Date().toISOString(),
            score: finalScore,
            maxScore: Number(params.maxScore) > finalScore ? Number(params.maxScore) : finalScore,
            subjects: completedSubjects,
            isSynced: isOnline,
        };
            ...
            if (isOnline) {
                ...
                if (userJson) {
                    ...
                    if (userEmail) {
                        const completedTestForFirestore = {
                            ...completedTest,
                            subjects: completedTest.subjects.map(s => ({ ...s, coverImage: null })),
                        };
                        const db = getFirestore(app);
                        const testDocRef = doc(db, 'users', userEmail, 'testHistory', completedTest.id);
                        await setDoc(testDocRef, completedTestForFirestore);
                        console.log('Cheated test result saved to Firestore successfully!');
                    } else {
                        console.log('Email not found in stored user data. Cannot save cheated test result to Firestore.');
                    }
                } else {
                    console.log('User not logged in or data not found in storage. Cannot save cheated test result to Firestore.');
                }
            } else {
                console.log('Offline. Cheated test saved locally, will sync later.');
            }
        } catch (e) {
            console.error("Failed to save cheated test result", e);
            Alert.alert("Save Error", "Could not save your test result to the cloud, but it has been saved on your device.");
        }

        router.replace({ pathname: '/test/GTResult', params: { score: String(finalScore), maxScore: String(completedTest.maxScore), testName: testName } });
    };
        ...
        <View style={{ flex: 1 }}>
            ...
            <SafeAreaView style={styles.safeArea}>
                <MotiView style={styles.header} from={{ translateY: -100 }} animate={{ translateY: 0 }} transition={{ type: 'spring', damping: 20 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 15 }}>
                        <TouchableOpacity onPress={() => router.back()} style={{ padding: 5 }}>
                            <Ionicons name="close-outline" size={28} color="#87B59A" />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={handleCheatSubmit} style={styles.cheatButton}>
                            <Text style={styles.cheatButtonText}>Score 800</Text>
                        </TouchableOpacity>
                    </View>
                    {currentSession && (
                        <View style={styles.headerSubject}>
                            <Image source={currentSession.coverImage} style={styles.subjectIcon} />
                            <Text style={styles.headerSubjectText} numberOfLines={1}>
                                {currentSession.subjectName}
                            </Text>
                        </View>
                    )}
                    <View style={styles.timer}>
                        <Ionicons name="time-outline" size={20} color="#87B59A" />
                ...
                <AnimatePresence exitBeforeEnter>
                    {examState === 'briefing' && currentSession &&
                        <BriefingScreen key="briefing" session={currentSession} onStart={() => setExamState('testing')} totalSections={sessions.length} timeLeft={secondsLeft} />
                    }
                    {examState === 'testing' && currentQuestion &&
                        <TestingScreen key={`testing-${globalQuestionIndex}`} question={currentQuestion} onAnswer={handleAnswer} globalQuestionNumber={globalQuestionIndex + 1} totalQuestions={totalQuestions} onNext={handleNext} onPrev={handlePrev} isLast={isLastQuestionOfAll} isFirst={isFirstQuestionOfAll} currentAnswer={answers[globalQuestionIndex]} />
                    }
                    {examState === 'review' && currentSession &&
                        <ReviewScreen key="review" onContinue={handleContinueFromReview} subjectBreakdown={allSubjectBreakdowns[currentSession.subjectId]} answers={answers} baseGlobalIndex={baseGlobalIndexForReview} isFinalReview={isFinalReview} />
                    }
                </AnimatePresence>
            </SafeAreaView>
        </View>
...
const styles = StyleSheet.create({
    ...
    header: {
        ...
        paddingVertical: 10,
        backgroundColor: 'rgba(18, 36, 25, 0.7)',
        borderBottomWidth: 1,
    ...
    infoRow: {
        ...
        marginBottom: 10,
        borderBottomWidth: 1,
        borderBottomColor: 'rgba(135, 181, 154, 0.1)',
        paddingBottom: 10
    },
    ...
    navButton: {
        ...
        alignItems: 'center',
        gap: 8,
        backgroundColor: 'rgba(135, 181, 154, 0.2)',
